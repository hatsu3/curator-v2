...
def handle_Index(the_class):
    ...
    replace_method(
        the_class, "update_vectors", replacement_update_vectors, ignore_missing=True
    )
    replace_method(
        the_class,
    ...
    replace_method(
        the_class, "add_sa_codes", replacement_add_sa_codes, ignore_missing=True
    )
    replace_method(
        the_class, "permute_entries", replacement_permute_entries, ignore_missing=True
    )
...
def handle_MultiTenantIndex(the_class):
    def replacement_add_vector(self, x):
        ...
        x : array_like
            ...
            `dtype` must be float32.
        tids : array_like
            List of IDs of the tenants that can access the vectors.
            This list is shared among all vectors added in the same batch.
        """
    ...
    def replacement_assign(self, x, k, tid, labels=None):
        """Find the k nearest neighbors of the set of vectors x in the index.
        This is the same as the `search` method, but discards the distances.
        ...
        k : int
            Number of nearest neighbors.
        tid : int
            ID of the tenant that is performing the search.
        labels : array_like, optional
            Labels array to store the results.
        ...
        self.assign_c(n, swig_ptr(x), tid, swig_ptr(labels), k)
        return labels

    def replacement_train(self, x, tid):
        """Trains the index on a representative set of vectors.
        The index must be trained before vectors can be added to it.
        ...
        x : array_like
            ...
            `dtype` must be float32.
        tid : int
            ID of the tenant that is training the index.
        """
        n, d = x.shape
        assert d == self.d
        x = np.ascontiguousarray(x, dtype="float32")
        self.train_c(n, swig_ptr(x), tid)

    def replacement_search(self, x, k, tid, *, params=None, D=None, I=None):
        """Find the k nearest neighbors of the set of vectors x in the index.
        ...
        self.search_c(n, swig_ptr(x), k, tid, swig_ptr(D), swig_ptr(I), params)
        return D, I

    def replacement_search_and_reconstruct(
        self, x, k, tid, *, params=None, D=None, I=None, R=None
    ):
        """Find the k nearest neighbors of the set of vectors x in the index,
        and return an approximation of these vectors.
        ...
        self.search_and_reconstruct_c(
            n, swig_ptr(x), k, tid, swig_ptr(D), swig_ptr(I), swig_ptr(R), params
        )
        return D, I, R
    ...
    def replacement_update_vectors(self, keys, x, tid):
        n = keys.size
        assert keys.shape == (n,)
        assert x.shape == (n, self.d)
        x = np.ascontiguousarray(x, dtype="float32")
        keys = np.ascontiguousarray(keys, dtype="int64")
        self.update_vectors_c(n, swig_ptr(keys), swig_ptr(x), tid)

    # No support passed-in for output buffers
    def replacement_range_search(self, x, thresh, tid, *, params=None):
        """Search vectors that are within a distance of the query vectors.
        ...
        res = RangeSearchResult(n)
        self.range_search_c(n, swig_ptr(x), thresh, tid, res, params)
        # get pointers and copy them
        lims = rev_swig_ptr(res.lims, n + 1).copy()
        nd = int(lims[-1])
        D = rev_swig_ptr(res.distances, nd).copy()
    ...
    def replacement_search_preassigned(
        self, x, k, tid, Iq, Dq, *, params=None, D=None, I=None
    ):
        """Find the k nearest neighbors of the set of vectors x in an IVF index,
        with precalculated coarse quantization assignment.
    ...
    def replacement_range_search_preassigned(
        self, x, thresh, tid, Iq, Dq, *, params=None
    ):
        """Search vectors that are within a distance of the query vectors.
        ...
        res = RangeSearchResult(n)
        self.range_search_preassigned_c(
            n, swig_ptr(x), thresh, tid, swig_ptr(Iq), swig_ptr(Dq), res
        )
        # get pointers and copy them
        lims = rev_swig_ptr(res.lims, n + 1).copy()
        nd = int(lims[-1])
    ...
    replace_method(the_class, "train", replacement_train)
    replace_method(the_class, "search", replacement_search)
    replace_method(the_class, "remove_vector", replacement_remove_vector)
    replace_method(the_class, "revoke_access", replacement_revoke_access)
    replace_method(the_class, "reconstruct", replacement_reconstruct)
    replace_method(the_class, "reconstruct_batch", replacement_reconstruct_batch)
    replace_method(the_class, "reconstruct_n", replacement_reconstruct_n)
    replace_method(the_class, "range_search", replacement_range_search)

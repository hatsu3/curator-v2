"""Build time plotting script using real evaluation results.

This script loads build time data from actual experiment results generated by
run_overall_results.sh instead of using hardcoded values. It handles different 
column patterns:

1. For most baselines: uses 'batch_insert_latency' column directly (adjusted for multi-threading)
2. For curator and similar: computes build time as:
   train_latency * num_threads (if exists, training uses multi-threading) + 
   num_vecs * insert_lat_avg (single-threaded) + 
   num_mds * access_grant_lat_avg (single-threaded)

Usage:
    # Plot build time for arXiv dataset only
    ...
    python -m benchmark.overall_results.plotting.build_time2 plot_construction_time \
        ...
        --datasets '["arxiv"]' \
        --output_path output/overall_results2/figs/build_time_arxiv.pdf
...
Requirements:
    ...
    - Each algorithm should have a results.csv file with appropriate timing columns
"""

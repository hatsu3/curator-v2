"""
Recall vs Latency Plotting for Overall Results

This module provides functionality to plot recall vs latency curves across different
selectivity levels using results generated by the run_overall_results.sh script.

Key Features:
- Automatically discovers and loads baseline results from run_overall_results.sh output structure
- Groups labels by selectivity percentiles for analysis with dataset-specific defaults
- Preprocesses raw benchmark results for efficient plotting
- Uses appropriate labels_per_group values: 100 for yfcc100m datasets, 10 for arxiv datasets

Directory Structure Expected:
The script expects results in the following structure from run_overall_results.sh:
    results_dir/
    ├── curator/
...
1. Basic usage:
   python -m benchmark.overall_results.plotting.recall_vs_latency2 \
...
2. Full control with custom options:
   python -m benchmark.overall_results.plotting.recall_vs_latency2 \
       plot_recall_vs_latency \
       --results_dir "output/overall_results2" \
       ...
       --labels_per_group 100 \
       --force_reprocess true

3. Preprocess results only:
   python -m benchmark.overall_results.plotting.recall_vs_latency2 \
...
Function Overview:
- plot_recall_vs_latency(): Main plotting function with full parameter control
- preprocess_all_baselines(): Preprocess raw results for faster subsequent plotting
- discover_baseline_results(): Auto-discover available baseline results
- aggregate_per_selectivity_results(): Process individual baseline results
...
Supported Baselines:
The script automatically recognizes and plots the following baselines:
- Curator
- Per-Label HNSW  
- Per-Label IVF
- Shared HNSW
...
Output:
- PDF plots with recall vs latency curves for each selectivity percentile
- Preprocessed CSV files for faster subsequent plotting
- Automatic legend generation and consistent styling across baselines
"""

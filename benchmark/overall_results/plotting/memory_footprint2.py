"""Memory footprint plotting script using real evaluation results.

This script loads memory usage data from actual experiment results generated by
run_overall_results.sh instead of using hardcoded values. It reads the index_size_kb
column from the results.csv files for each algorithm and dataset.
...
Usage:
    ...
    python -m benchmark.overall_results.plotting.memory_footprint2 plot_memory_footprint \
        ...
        --datasets '["arxiv"]' \
        --output_path output/overall_results2/figs/memory_footprint_arxiv.pdf \
        --subtract_vectors
...
Requirements:
    ...
    - Each algorithm should have a results.csv file with index_size_kb column
"""

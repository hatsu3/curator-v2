"""Update performance plotting script using real evaluation results.

This script loads update performance data from actual experiment results generated by
run_overall_results.sh instead of using hardcoded file paths. It handles different 
algorithms and extracts the relevant update operation latencies:

- insert_lat_avg: Average vector insertion latency
- access_grant_lat_avg: Average label insertion latency  
- delete_lat_avg: Average vector deletion latency (Curator only - from previous results)
- revoke_access_lat_avg: Average label deletion latency (Curator only - from previous results)

IMPORTANT LIMITATION:
Vector deletion (delete_lat_avg) is currently disabled in the unified_curator branch
because IndexFlat storage doesn't support individual vector removal. Both deletion
performance values for Curator are loaded from previous experiment results and should
be updated when deletion is reimplemented with a different storage mechanism.

Usage:
    # Plot update performance for arXiv dataset only
...
Requirements:
    - Experiment results should be available in the specified output directory
    - Results should be generated using run_overall_results.sh
    - Each algorithm should have a results.csv file with appropriate update timing columns
"""

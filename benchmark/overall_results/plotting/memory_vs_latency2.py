"""
Memory vs Latency Plotting Script

This script provides two main plotting functions for analyzing memory footprint, build time, 
and latency relationships across different baselines and selectivity levels.

Key Features:
- Computes recall vs latency curves for each baseline under given selectivity thresholds
- Uses linear interpolation to estimate latency at target recall (default: 0.9)
- Loads memory footprint and build time data from experiment results
- Generates scatter plots with baseline name annotations
- Supports configurable annotation offsets via YAML configuration file
- Ensures consistent color mapping across datasets for the same baselines
- Two plotting modes: cross-dataset comparison and per-selectivity analysis

Main Functions:

1. plot_memory_vs_latency_vs_build_time():
   - Plots memory vs build time with latency as marker size
   - Compares multiple datasets side-by-side under a single selectivity threshold
   - Supports configurable annotation positioning

2. plot_memory_vs_latency_per_selectivity():
   - Plots memory vs build time with latency as marker size for each selectivity group
   - Creates subplots for different selectivity percentiles (similar to recall_vs_latency2.py)
   - Analyzes a single dataset across multiple selectivity levels

Usage Examples:

1. Cross-dataset comparison (default mode):
...
2. Per-selectivity analysis for a single dataset:
   python -m benchmark.overall_results.plotting.memory_vs_latency2 plot_memory_vs_latency_per_selectivity \
       --results_dir "output/overall_results2" \
       --dataset_name "yfcc100m" \
       --output_path "output/overall_results2/figs/memory_vs_latency_per_selectivity_yfcc100m.pdf"

3. Custom selectivity percentiles:
   python -m benchmark.overall_results.plotting.memory_vs_latency2 plot_memory_vs_latency_per_selectivity \
       --results_dir "output/overall_results2" \
       --dataset_name "arxiv" \
       --output_path "output/overall_results2/figs/memory_vs_latency_arxiv.pdf" \
       --percentiles "[0.01, 0.50, 1.00]" \
       --target_recall 0.95

4. Generate annotation configuration file:
   python -m benchmark.overall_results.plotting.memory_vs_latency2 generate_annotation_config \
       --results_dir "output/overall_results2" \
       --output_path "benchmark/overall_results/plotting/my_annotation_config.yaml" \
       --dataset_names "['yfcc100m', 'arxiv']"

5. Single dataset in cross-dataset mode:
   python -m benchmark.overall_results.plotting.memory_vs_latency2 plot_memory_vs_latency_vs_build_time \
       --results_dir "output/overall_results2" \
       --dataset_names "['yfcc100m']" \
       --output_path "output/overall_results2/figs/yfcc100m_memory_vs_latency.pdf" \
       --annotation_config_path "benchmark/overall_results/plotting/annotation_offsets_sample.yaml"

Configuration Files:
- Annotation configuration files control the positioning of text labels on the plot
- Use YAML format with structure: annotation_offsets -> dataset_name -> baseline_name
- Each baseline can have x_offset, y_offset, ha (horizontal alignment), va (vertical alignment)
- Generate a sample config file using the generate_annotation_config command

Expected Directory Structure:
<results_dir>/
├── curator/
│   └── <dataset_key>_test<test_size>/
│       └── results.csv
├── per_label_hnsw/
│   └── <dataset_key>_test<test_size>/
│       └── results.csv
...
Supported Datasets:
- "yfcc100m": Uses dataset_key="yfcc100m-10m", test_size=0.001
- "arxiv": Uses dataset_key="arxiv-large-10", test_size=0.005

Supported Baselines:
- Curator, Per-Label HNSW, Per-Label IVF, Shared HNSW, Shared IVF
- Parlay IVF, Filtered DiskANN, ACORN-1, ACORN-γ

Requirements:
- All algorithms must have valid build time data
- Results files must contain query_latencies and query_recalls columns
- Memory footprint data must be available in the results directory
"""
...
from benchmark.overall_results.plotting.build_time2 import load_algorithm_build_time
from benchmark.overall_results.plotting.memory_footprint2 import (
    load_algorithm_memory_usage,
)
from benchmark.overall_results.plotting.recall_vs_latency2 import (
    discover_baseline_results,
    preprocess_all_baselines,
)
from benchmark.profiler import Dataset

# Dataset-specific configuration (reused from recall_vs_latency2.py)
...
def plot_memory_vs_latency_per_selectivity(
    ...
    Creates subplots for each selectivity percentile, similar to recall_vs_latency2.py.
